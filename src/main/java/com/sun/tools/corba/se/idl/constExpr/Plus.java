...
/* loaded from: target.jar:com/sun/tools/corba/se/idl/constExpr/Plus.class */
public class Plus extends BinaryExpr {
    /* JADX INFO: Access modifiers changed from: protected */
    public Plus(Expression expression, Expression expression2) {
        super(Marker.ANY_NON_NULL_MARKER, expression, expression2);
    }
    ...
    public Object evaluate() throws EvaluationException {
        try {
            ...
            if (z && z2) {
                ...
                if (z || z2) {
                    throw new EvaluationException(Util.getMessage("EvaluationException.1", new String[]{Util.getMessage("EvaluationException.plus"), left().value().getClass().getName(), right().value().getClass().getName()}));
                }
                value(((BigInteger) number).add((BigInteger) number2));
            }
            return value();
