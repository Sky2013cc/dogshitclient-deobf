...
/* loaded from: target.jar:jdk/nashorn/internal/runtime/options/Options.class */
public final class Options {
    ...
    static {
        ...
        while (keys.hasMoreElements()) {
            ...
            StringTokenizer st = new StringTokenizer(key, Constants.NAME_SEPARATOR);
            ...
            public Boolean run() {
                try {
                    ...
                    if (property != null || defValue == null) {
                        return Boolean.valueOf((property == null || com.sun.tools.internal.ws.wsdl.parser.Constants.FALSE.equalsIgnoreCase(property)) ? false : true);
                    }
                    return defValue;
    ...
    private String key(String shortKey) {
        ...
        while (true) {
            key = str;
            if (!key.startsWith(TagletManager.ALT_SIMPLE_TAGLET_OPT_SEPARATOR)) {
                break;
            }
            str = key.substring(1, key.length());
        }
        String key2 = key.replace(TagletManager.ALT_SIMPLE_TAGLET_OPT_SEPARATOR, Constants.NAME_SEPARATOR);
        ...
        while (!argList.isEmpty()) {
            ...
            if (!arg.isEmpty()) {
                if ("--".equals(arg)) {
                    ...
                    argList.clear();
                } else if (!arg.startsWith(TagletManager.ALT_SIMPLE_TAGLET_OPT_SEPARATOR) || arg.length() == 1) {
                    this.files.add(arg);
    ...
    /* JADX INFO: Access modifiers changed from: private */
    /* loaded from: target.jar:jdk/nashorn/internal/runtime/options/Options$IllegalOptionException.class */
    ...
    /* JADX INFO: Access modifiers changed from: private */
    /* loaded from: target.jar:jdk/nashorn/internal/runtime/options/Options$ParsedArg.class */
    public static class ParsedArg {
        ...
        ParsedArg(String argument) {
            ...
            if (Constants.IDL_BOOLEAN.equals(this.template.getType())) {
                this.value = com.sun.tools.internal.ws.wsdl.parser.Constants.TRUE;
            }
        }
    }
}
