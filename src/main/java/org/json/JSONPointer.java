...
/* loaded from: target.jar:org/json/JSONPointer.class */
public class JSONPointer {
    private static final String ENCODING = "utf-8";
    private final List<String> refTokens;

    /* loaded from: target.jar:org/json/JSONPointer$Builder.class */
    ...
    public JSONPointer(String pointer) {
        ...
        } else if (pointer.startsWith(RuntimeConstants.SIG_PACKAGE)) {
            refs = pointer.substring(1);
        } else {
            throw new IllegalArgumentException("a JSON pointer should start with '/' or '#/'");
        }
        this.refTokens = new ArrayList();
        for (String token : refs.split(RuntimeConstants.SIG_PACKAGE)) {
    ...
    private String unescape(String token) {
        return token.replace("~1", RuntimeConstants.SIG_PACKAGE).replace("~0", "~").replace("\\\"", OperatorName.SHOW_TEXT_LINE_AND_SPACE).replace("\\\\", "\\");
    }
    ...
    private String escape(String token) {
        return token.replace("~", "~0").replace(RuntimeConstants.SIG_PACKAGE, "~1").replace("\\", "\\\\").replace(OperatorName.SHOW_TEXT_LINE_AND_SPACE, "\\\"");
    }
