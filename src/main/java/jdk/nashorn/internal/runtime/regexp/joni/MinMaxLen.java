...
/* loaded from: target.jar:jdk/nashorn/internal/runtime/regexp/joni/MinMaxLen.class */
final class MinMaxLen {
    ...
    public static String distanceRangeToString(int a, int b) {
        String s;
        String s2 = (a == Integer.MAX_VALUE ? "inf" : "" + RuntimeConstants.SIG_METHOD + a + RuntimeConstants.SIG_ENDMETHOD) + TagletManager.ALT_SIMPLE_TAGLET_OPT_SEPARATOR;
        if (b == Integer.MAX_VALUE) {
            s = s2 + "inf";
        } else {
            s = s2 + RuntimeConstants.SIG_METHOD + b + RuntimeConstants.SIG_ENDMETHOD;
        }
        return s;
    }
}
