...
/* loaded from: target.jar:jdk/internal/dynalink/support/Lookup.class */
public class Lookup {
    ...
    public MethodHandle findGetter(Class<?> refc, String name, Class<?> type) {
        try {
            ...
            IllegalAccessError ee = new IllegalAccessError("Failed to access getter for field " + refc.getName() + Constants.NAME_SEPARATOR + name + " of type " + type.getName());
            ee.initCause(e);
            throw ee;
        } catch (NoSuchFieldException e2) {
            NoSuchFieldError ee2 = new NoSuchFieldError("Failed to find getter for field " + refc.getName() + Constants.NAME_SEPARATOR + name + " of type " + type.getName());
            ee2.initCause(e2);
            throw ee2;
        }
    }
