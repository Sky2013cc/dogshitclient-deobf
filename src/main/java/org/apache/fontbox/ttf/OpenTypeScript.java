...
/* loaded from: target.jar:org/apache/fontbox/ttf/OpenTypeScript.class */
public final class OpenTypeScript {
    ...
    static {
        ...
        try {
            InputStream resourceAsStream = OpenTypeScript.class.getResourceAsStream("/org/apache/fontbox/unicode/Scripts.txt");
            Throwable th = null;
            try {
            ...
            LOG.warn("Could not parse Scripts.txt, mirroring char map will be empty: " + e.getMessage(), e);
        }
    }
    ...
    private static void parseScriptsFile(InputStream inputStream) throws IOException {
        Map<int[], String> unicodeRanges = new TreeMap<>((Comparator<? super int[]>) (o1, o2) -> {
        ...
        try {
            ...
            while (true) {
                ...
                if (s.length() >= 2) {
                    StringTokenizer st = new StringTokenizer(s, RuntimeConstants.SIG_ENDCLASS);
                    ...
                    if (nFields >= 2) {
                        ...
                        if (rangeDelim == -1) {
                            int parseInt = Integer.parseInt(characters, 16);
                            range[1] = parseInt;
                            range[0] = parseInt;
                        } else {
