...
/* loaded from: target.jar:org/apache/pdfbox/pdmodel/encryption/SecurityHandler.class */
public abstract class SecurityHandler<T_POLICY extends ProtectionPolicy> {
    ...
    private T_POLICY protectionPolicy;
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void setDecryptMetadata(boolean decryptMetadata) {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void encryptDataRC4(byte[] finalKey, InputStream input, OutputStream output) throws IOException {
        this.rc4.setKey(finalKey);
        this.rc4.write(input, output);
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public void encryptDataRC4(byte[] finalKey, byte[] input, OutputStream output) throws IOException {
        this.rc4.setKey(finalKey);
        this.rc4.write(input, output);
    }
    ...
    /* JADX WARN: Type inference failed for: r0v47, types: [java.lang.Throwable, java.io.InputStream] */
    /* JADX WARN: Type inference failed for: r0v58, types: [byte[], java.io.InputStream] */
    public void decryptStream(COSStream stream, long objNum, long genNum) throws IOException {
        ...
        if ((this.decryptMetadata || !COSName.METADATA.equals(type)) && !COSName.XREF.equals(type)) {
            if (COSName.METADATA.equals(type)) {
                ?? createRawInputStream = stream.createRawInputStream();
                Throwable th = null;
                try {
                    try {
                        ?? r0 = new byte[10];
                        long isResult = IOUtils.populateBuffer(createRawInputStream, r0);
                        if (Long.compare(isResult, r0.length) != 0) {
                            LOG.debug("Tried reading " + r0.length + " bytes but only " + isResult + " bytes read");
                        }
                        if (createRawInputStream != 0) {
                            if (0 != 0) {
                                try {
                        ...
                        if (Arrays.equals((byte[]) r0, "<?xpacket ".getBytes(StandardCharsets.ISO_8859_1))) {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public T_POLICY getProtectionPolicy() {
