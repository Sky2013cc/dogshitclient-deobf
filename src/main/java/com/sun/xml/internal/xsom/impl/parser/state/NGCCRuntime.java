...
/* loaded from: target.jar:com/sun/xml/internal/xsom/impl/parser/state/NGCCRuntime.class */
public class NGCCRuntime implements ContentHandler, NGCCEventSource {
    ...
    public void unexpectedX(String token) throws SAXException {
        throw new SAXParseException(MessageFormat.format("Unexpected {0} appears at line {1} column {2}", token, new Integer(getLocator().getLineNumber()), new Integer(getLocator().getColumnNumber())), getLocator());
    }
