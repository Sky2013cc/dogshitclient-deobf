...
/* loaded from: target.jar:com/formdev/flatlaf/util/SystemInfo.class */
public class SystemInfo {
    ...
    static {
        ...
        isJetBrainsJVM = System.getProperty("java.vm.vendor", OpenTypeScript.UNKNOWN).toLowerCase(Locale.ENGLISH).contains("jetbrains");
        ...
        if (isWindows_10_orLater) {
            if (scanVersion(StringUtils.removeLeading(osName, "windows ")) < toVersion(11, 0, 0, 0)) {
    ...
    public static long toVersion(int major, int minor, int micro, int patch) {
        return (major << 48) + (minor << 32) + (micro << 16) + patch;
    }
}
