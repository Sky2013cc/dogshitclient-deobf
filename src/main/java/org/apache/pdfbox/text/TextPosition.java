...
/* loaded from: target.jar:org/apache/pdfbox/text/TextPosition.class */
public final class TextPosition {
    ...
    private float getXRot(float rotation) {
        ...
        if (Float.compare(rotation, 90.0f) == 0) {
            return this.textMatrix.getTranslateY();
        }
        if (Float.compare(rotation, 180.0f) == 0) {
            return this.pageWidth - this.textMatrix.getTranslateX();
        }
        if (Float.compare(rotation, 270.0f) == 0) {
    ...
    public float getYDirAdj() {
        float dir = getDir();
        if (Float.compare(dir, 0.0f) == 0 || Float.compare(dir, 180.0f) == 0) {
    ...
    private float getWidthRot(float rotation) {
        if (Float.compare(rotation, 90.0f) == 0 || Float.compare(rotation, 270.0f) == 0) {
