...
/* loaded from: target.jar:com/formdev/flatlaf/icons/FlatCheckBoxIcon.class */
public class FlatCheckBoxIcon extends FlatAbstractIcon {
    ...
    protected static Color getUIColor(String key, String style) {
        Color color;
        if (style != null && (color = UIManager.getColor(styleKey(key, style))) != null) {
            return color;
        }
    ...
    public static float getUIFloat(String key, float defaultValue, String style) {
        if (style != null) {
    ...
    protected void paintIcon(Component c, Graphics2D g) {
        ...
        if (!selected) {
            f = this.borderWidth;
        } else if (this.disabledSelectedBorderWidth == Float.MIN_VALUE || c.isEnabled()) {
            f = this.selectedBorderWidth != Float.MIN_VALUE ? this.selectedBorderWidth : this.borderWidth;
        } else {
            f = this.disabledSelectedBorderWidth;
        }
        float bw = f;
        ...
        paintBorder(c, g, bw);
        Color bg = FlatUIUtils.deriveColor(getBackground(c, selected), selected ? this.selectedBackground : this.background);
        if (bg.getAlpha() < 255) {
    ...
    protected void paintFocusBorder(Component c, Graphics2D g) {
        ...
        float arcwh = this.arc + (this.focusWidth * 2.0f);
    ...
    protected void paintBackground(Component c, Graphics2D g, float borderWidth) {
        ...
        float arcwh = this.arc - borderWidth;
        g.fill(new RoundRectangle2D.Float(1.0f + borderWidth, borderWidth, wh, wh, arcwh, arcwh));
    }
    ...
    protected Color getBorderColor(Component c, boolean selected) {
        return FlatButtonUI.buttonStateColor(c, selected ? this.selectedBorderColor : this.borderColor, (!selected || this.disabledSelectedBorderColor == null) ? this.disabledBorderColor : this.disabledSelectedBorderColor, (!selected || this.focusedSelectedBorderColor == null) ? this.focusedBorderColor : this.focusedSelectedBorderColor, (!selected || this.hoverSelectedBorderColor == null) ? this.hoverBorderColor : this.hoverSelectedBorderColor, (!selected || this.pressedSelectedBorderColor == null) ? this.pressedBorderColor : this.pressedSelectedBorderColor);
    }

    protected Color getBackground(Component c, boolean selected) {
        return FlatButtonUI.buttonStateColor(c, selected ? this.selectedBackground : this.background, (!selected || this.disabledSelectedBackground == null) ? this.disabledBackground : this.disabledSelectedBackground, (!selected || this.focusedSelectedBackground == null) ? this.focusedBackground : this.focusedSelectedBackground, (!selected || this.hoverSelectedBackground == null) ? this.hoverBackground : this.hoverSelectedBackground, (!selected || this.pressedSelectedBackground == null) ? this.pressedBackground : this.pressedSelectedBackground);
    }
