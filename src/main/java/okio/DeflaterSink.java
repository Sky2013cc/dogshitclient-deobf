...
/* loaded from: target.jar:okio/DeflaterSink.class */
public final class DeflaterSink implements Sink {
    ...
    private final void deflate(boolean syncFlush) {
        ...
        while (true) {
            ...
            if (syncFlush) {
                try {
                    deflate = this.deflater.deflate(s.data, s.limit, 8192 - s.limit, 2);
                } catch (NullPointerException npe) {
                    throw new IOException("Deflater already closed", npe);
                }
            } else {
                deflate = this.deflater.deflate(s.data, s.limit, 8192 - s.limit);
            }
            int deflated = deflate;
            if (deflated > 0) {
                s.limit += deflated;
