...
/* loaded from: target.jar:org/apache/pdfbox/pdmodel/interactive/digitalsignature/COSFilterInputStream.class */
public class COSFilterInputStream extends FilterInputStream {
    ...
    private void calculateRanges(int[] byteRange) {
        ...
        for (int i = 0; i < byteRange.length / 2; i++) {
            int[] iArr = new int[2];
            iArr[0] = byteRange[i * 2];
            iArr[1] = byteRange[i * 2] + byteRange[(i * 2) + 1];
            this.ranges[i] = iArr;
        }
        this.range = -1;
    }
