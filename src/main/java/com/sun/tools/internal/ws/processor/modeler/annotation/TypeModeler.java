...
/* loaded from: target.jar:com/sun/tools/internal/ws/processor/modeler/annotation/TypeModeler.class */
final class TypeModeler {
    ...
    public static Collection<DeclaredType> collectInterfaces(TypeElement type) {
        Collection<DeclaredType> interfaces = type.getInterfaces();
    ...
    public static TypeMirror getHolderValueType(TypeMirror type, TypeElement defHolder, ProcessingEnvironment env) {
        ...
        if (typeElement != null && isSubElement(typeElement, defHolder) && type.getKind().equals(TypeKind.DECLARED)) {
            Collection<? extends TypeMirror> argTypes = ((DeclaredType) type).getTypeArguments();
            if (argTypes.size() == 1) {
                return (TypeMirror) argTypes.iterator().next();
            }
            if (argTypes.isEmpty() && (member = getValueMember(typeElement)) != null) {
    ...
    public static VariableElement getValueMember(TypeElement type) {
        ...
        Iterator it = ElementFilter.fieldsIn(type.getEnclosedElements()).iterator();
        while (true) {
            if (!it.hasNext()) {
                break;
            }
            VariableElement field = (VariableElement) it.next();
    ...
    public static boolean isSubElement(TypeElement d1, TypeElement d2) {
        ...
        if (d1.getKind().equals(ElementKind.CLASS)) {
            DeclaredType superclass = d1.getSuperclass();
            if (!superclass.getKind().equals(TypeKind.NONE)) {
                superClassDecl = (TypeElement) superclass.asElement();
                if (superClassDecl.equals(d2)) {
                    return true;
                }
            }
        }
        for (DeclaredType declaredSuperIntf : d1.getInterfaces()) {
            if (declaredSuperIntf.asElement().equals(d2) || isSubElement(declaredSuperIntf.asElement(), d2)) {
                return true;
            }
            if (superClassDecl != null && isSubElement(superClassDecl, d2)) {
                return true;
            }
        }
        return false;
