...
/* loaded from: target.jar:com/sun/tools/corba/se/idl/constExpr/Divide.class */
public class Divide extends BinaryExpr {
    /* JADX INFO: Access modifiers changed from: protected */
    public Divide(Expression expression, Expression expression2) {
        super(RuntimeConstants.SIG_PACKAGE, expression, expression2);
    }
    ...
    public Object evaluate() throws EvaluationException {
        try {
            ...
            if (z && z2) {
                ...
                if (z || z2) {
                    throw new EvaluationException(Util.getMessage("EvaluationException.1", new String[]{Util.getMessage("EvaluationException.divide"), left().value().getClass().getName(), right().value().getClass().getName()}));
                }
                value(((BigInteger) number).divide((BigInteger) number2));
            }
            return value();
