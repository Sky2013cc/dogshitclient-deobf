...
/* loaded from: target.jar:org/spongepowered/tools/obfuscation/mirror/TypeUtils.class */
public abstract class TypeUtils {
    ...
    public static String getJavaSignature(Element element) {
        if (element instanceof ExecutableElement) {
            ...
            StringBuilder desc = new StringBuilder().append(RuntimeConstants.SIG_METHOD);
            boolean extra = false;
    ...
    public static String getTypeName(TypeMirror type) {
        switch (AnonymousClass1.$SwitchMap$javax$lang$model$type$TypeKind[type.getKind().ordinal()]) {
            case 1:
                return getTypeName(((ArrayType) type).getComponentType()) + _UrlKt.PATH_SEGMENT_ENCODE_SET_URI;
            case 2:
    ...
    public static String getInternalName(TypeMirror type) {
        switch (AnonymousClass1.$SwitchMap$javax$lang$model$type$TypeKind[type.getKind().ordinal()]) {
            case 1:
                return RuntimeConstants.SIG_ARRAY + getInternalName(((ArrayType) type).getComponentType());
            case 2:
                return "L" + getInternalName((DeclaredType) type) + RuntimeConstants.SIG_ENDCLASS;
            case 3:
                return "L" + getInternalName(getUpperBound(type)) + RuntimeConstants.SIG_ENDCLASS;
            case 4:
                return Constants.OBJECT;
            case 5:
                return RuntimeConstants.SIG_BOOLEAN;
            case 6:
                return "B";
            case 7:
                return "C";
        ...
        while (true) {
            ...
            if (parent != null) {
                if (parent instanceof TypeElement) {
                    reference.insert(0, "$").insert(0, (CharSequence) parent.getSimpleName());
                } else if (parent instanceof PackageElement) {
                    reference.insert(0, RuntimeConstants.SIG_PACKAGE).insert(0, ((PackageElement) parent).getQualifiedName().toString().replace('.', '/'));
                }
                enclosingElement = parent.getEnclosingElement();
            } else {
    ...
    /* loaded from: target.jar:org/spongepowered/tools/obfuscation/mirror/TypeUtils$1.class */
