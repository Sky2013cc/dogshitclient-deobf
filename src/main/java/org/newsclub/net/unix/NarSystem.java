...
/* loaded from: target.jar:org/newsclub/net/unix/NarSystem.class */
public final class NarSystem {
    ...
    private static String[] getAOLs() {
        String ao = System.getProperty("os.arch") + TagletManager.ALT_SIMPLE_TAGLET_OPT_SEPARATOR + System.getProperty("os.name").replaceAll(" ", "");
    ...
    private static File getUnpackedLibPath(ClassLoader loader, String[] aols, String fileName, String mapped) {
        ...
        String prefix = path.substring(0, path.length() - classPath.length()) + "../nar/" + fileName + TagletManager.ALT_SIMPLE_TAGLET_OPT_SEPARATOR;
        for (String aol : aols) {
